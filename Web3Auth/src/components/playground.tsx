...
export const Playground = ({ children }: IPlaygroundProps) => {
  ...
  const readItems = async (contractAddress: string, contractABI: any): Promise<string> => {
    if (!provider) {
      uiConsole("provider not initialized yet");
      return;
    }
    const message = await walletProvider.readItems(contractAddress, contractABI);
    uiConsole(message);
  };

  const readTokens = async (contractAddress: string, contractABI: any): Promise<string> => {
    if (!provider) {
      uiConsole("provider not initialized yet");
      return;
    }
    const message = await walletProvider.readTokens(contractAddress, contractABI);
    uiConsole(message);
  };
  ...
  const sellItem = async (contractAddress: string, contractABI: any, updatedValue: string): Promise<any> => {
    if (!provider) {
      uiConsole("provider not initialized yet");
      return;
    }
    uiConsole("Loading");
    const receipt = await walletProvider.sellItems(contractAddress, contractABI, updatedValue);
    
    uiConsole(receipt);
  };

  const sellTokens = async (contractAddress: string, contractABI: any, updatedValue: string): Promise<any> => {
    if (!provider) {
      uiConsole("provider not initialized yet");
      return;
    }
    uiConsole("Loading");
    const receipt = await walletProvider.sellTokens(contractAddress, contractABI, updatedValue);
