...
```bash
# For Linux
docker build -t Eth_Global:latest .

# For Mac with Apple Silicon chips
docker buildx build --platform linux/amd64,linux/arm64 -t Eth_Global:latest .
```
...
docker run --env-file .env -v ./db.sqlite:/app/db.sqlite -p <HOST_PORT>:<CONTAINER_PORT> --name=Eth_Global -it Eth_Global:latest

# If using other URI based datastores
docker run --env-file .env -p <HOST_PORT>:<CONTAINER_PORT> --name=Eth_Global -it Eth_Global:latest
```
