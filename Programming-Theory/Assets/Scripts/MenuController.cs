...
//        tokenCountText.text = "Tokens Remaining: " + GameManager.Instance.playerTokens;
    ...
    // Method to be called to open the URL
    ...
    public void BuyCar()
    {
        ...
        string redUrl = "http://localhost:5173/contract/" + carType.ToString();
        RedirectToWebsite(redUrl);
        //if (carType.ToString() == "Pickup")
        //{
        //    //redirecr to website with pickup
    ...
    public void SellCar()
    {
        ...
        // Check if player has enough tokens and car is not already owned
        string redUrl = "http://localhost:5173/contract/";
        


        
        
        string carToEnable = carType.ToString();

        List<string> items=sc.items;

        //int a = items.FindIndex(carToEnable);
       

        for(int i=0;i<items.Count;i++)
        {
            if (items[i] == carToEnable) {
                redUrl += i.ToString();
            }
